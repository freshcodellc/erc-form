...
const validationSchemaEZ = Yup.object()
  .shape({
    forgivenessAmountAgreement: Yup.bool()
      .oneOf([true], 'Must agree.')
      .required('Required'),
    fraudAgreement: Yup.bool()
      .oneOf([true], 'Must agree.')
      .required('Required'),
    noReducedSalariesAgreement: Yup.bool()
      .oneOf([true], 'Must agree.')
      .required('Required'),
    forgivenessVerificationAgreement: Yup.bool()
      .oneOf([true], 'Must agree.')
  ...
  .test('oneOfTwo', null, obj => {
    if (obj.noReducedEmployeesAgreement || obj.decreasedOperationsAgreement) {
      return true
    }
...
function ForgivenessRecertify() {
  ...
  const handleCertSubmit = async (values, actions) => {
    ...
    try {
      const response = await axios.post(
        `${config.API_URL}/api/forgiveness/recertify`,
        {
          ...values,
          bank: appData.bank.slug,
          email: appData.customer.email,
          form: appData.forgiveness.form,
          uniqueId: appData.application.uniqueId,
        }
      )
      setSuccess(true)
  ...
  const changes = get(appData, 'forgiveness.changes', [])
  const isEZ = get(appData, 'forgiveness.form', '3508') === '3508EZ'
    ...
    return (
      <Layout>
        <div css={formStyle}>
          ...
          <br />
          <p>
            Your bank will reach out to you if anything further is required.
            Please feel free to reach out to your funding bank if you have any
  ...
  return (
    <Layout>
      <div css={formStyle}>
        ...
        <p css={text}>
          Because changes were required for your forgiveness application to be
          approved by the SBA, we will need you to complete this page before we
          submit your application again. Please review the changes below, then
          check all the agreements and certifications and submit.
        </p>
        <div css={changeList}>
          <ul>
            {changes &&
              changes.length > 0 &&
              _values(changes[0].changes).map((change, i) => (
                <React.Fragment key={i}>
                  {change.type !== 'array' ? (
                    <li>
                      {`${change.label} was changed from ${change.old} to ${change.new}`}
                    </li>
                  ) : null}
                </React.Fragment>
              ))}
        ...
        <div css={formWrap}>
          <Formik
            initialValues={isEZ ? initialValuesEZ : initialValues}
            validationSchema={isEZ ? validationSchemaEZ : validationSchema}
...
COVID-19."
                  ...
                  <p>
                    ...
                    evaluated in accordance with the PPP regulations and
                    guidance issued by SBA through the date of this application.
                    SBA may direct a lender to disapprove the Borrower’s loan
                    forgiveness application if SBA determines that the Borrower
                  ...
                  {error && (
                    <span
                      ...
                      `}
                    >
                      There was an error submitting your forgiveness
                      re-certification. Please contact {bank.name} for
                      assistance.
                    </span>
                  )}
                </form>
...
export default ForgivenessRecertify
